...
public class OpenCvRaspberryPiCamera implements CubeColorInspector{
    ...
    public static char classifyColor(Mat square) {
        ...
        for (int rows = 0; rows < labSquare.rows(); rows++) {//TODO use the for loop to go through every row
            for (int cols = 0; cols < labSquare.cols(); cols++) {//TODO use a for loop to go through every column
                int pixelL = indexer.get(rows, cols, 0);//TODO get the L value of that pixel by putting row, col, and 0 because L is the first value
                int pixelA = indexer.get(rows, cols, 1);//TODO A is second value, so 1
                int pixelB = indexer.get(rows, cols, 2);//TODO B is 2
    ...
    public static char classifyColorDeltaELab(int l, int a, int b){
        ...
        if(autoTune){
            realReferenceColors[face][piece][color][0] = l;//TODO set realReferenceColors for the current indexes face, piece, color, and 0 (because the "l" value) to l;
            realReferenceColors[face][piece][color][1] = a;//TODO same for "a" value except it is 1 instead of 0 because it's the second value
            realReferenceColors[face][piece][color][2] = b;//TODO b is 2 because 3rd value
            piece++;
            return 'U';
        }else{
